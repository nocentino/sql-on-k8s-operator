...
const namespace = "sql-on-k8s-operator-system"

// serviceAccountName created for the project
const serviceAccountName = "sql-on-k8s-operator-controller-manager"

// metricsServiceName is the name of the metrics service of the project
const metricsServiceName = "sql-on-k8s-operator-controller-manager-metrics-service"

// metricsRoleBindingName is the name of the RBAC that will be created to allow get the metrics data
const metricsRoleBindingName = "sql-on-k8s-operator-metrics-binding"

var _ = Describe("Manager", Ordered, func() {
	var controllerPodName string

	// The operator is deployed by BeforeSuite in e2e_suite_test.go.
	// AfterAll cleans up resources that were created exclusively by this spec block.
	AfterAll(func() {
	...
	Context("Manager", func() {
		...
		It("should ensure the metrics endpoint is serving metrics", func() {
			...
			cmd := exec.Command("kubectl", "create", "clusterrolebinding", metricsRoleBindingName,
				"--clusterrole=sql-on-k8s-operator-metrics-reader",
			...
			verifyWebhookEndpointsReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "endpointslices.discovery.k8s.io", "-n", namespace,
					"-l", "kubernetes.io/service-name=sql-on-k8s-operator-webhook-service",
			...
			verifyValidatingWebhookReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "validatingwebhookconfigurations.admissionregistration.k8s.io",
					"sql-on-k8s-operator-validating-webhook-configuration",
